...
class Detect(nn.Module):
    ...
    def __init__(self, nc=14, anchors=(), ch=(), inplace=True):  # detection layer
        super().__init__()
...
class Decoupled_Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            x[i] = torch.cat(
                [self.cls_head(self.cls_c2(self.cls_c1(x[i]))), self.reg_head(reg_heads), self.obj_head(reg_heads)], 1)
...
class ASFF_Detect(nn.Module):  # add ASFFV5 layer and Rfb
    ...
    def __init__(self, nc=14, anchors=(), ch=(), multiplier=0.5, rfb=False, inplace=True):  # detection layer
        super().__init__()
        self.nc = nc  # number of classes
        self.no = nc + 5  # number of outputs per anchor
        self.nl = len(anchors)  # number of detection layers
...
class ASFF_Detect_4head(nn.Module):  # add ASFFV5 layer and Rfb
    stride = None  # strides computed during build
    onnx_dynamic = False  # ONNX export parameter
    ...
    def __init__(self, nc=14, anchors=(), ch=(), multiplier=0.5, rfb=False, inplace=True):  # detection layer
        ...
        self.na = len(anchors[0]) // 2  # number of anchors
        self.grid = [torch.zeros(1)] * self.nl  # init grid
        self.l0_fusion = ASFFV5_4head(level=0, multiplier=multiplier, rfb=rfb)
        self.l1_fusion = ASFFV5_4head(level=1, multiplier=multiplier, rfb=rfb)
        self.l2_fusion = ASFFV5_4head(level=2, multiplier=multiplier, rfb=rfb)
        self.l3_fusion = ASFFV5_4head(level=3, multiplier=multiplier, rfb=rfb)
        self.anchor_grid = [torch.zeros(1)] * self.nl  # init anchor grid
        self.register_buffer('anchors', torch.tensor(anchors).float().view(self.nl, -1, 2))  # shape(nl,na,2)
        self.m = nn.ModuleList(nn.Conv2d(x, self.no * self.na, 1) for x in ch)  # output conv
        self.inplace = inplace  # use in-place ops (e.g. slice assignment)
...
class Model(nn.Module):
    def __init__(self, cfg='yolov5s_bifpn.yaml', ch=3, nc=None,
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        ...
        if m in [Conv, DCConv, GhostConv, Bottleneck, GhostBottleneck, SPP, SPPF, DWConv, MixConv2d, Focus,
                 BottleneckCSP, CBAM, CoordAtt, CrossConv, C3, CTR3, Involution, C3SPP, C3Ghost, CARAFE, ResBlock,
                 C3Deform, C3New, NewAttention, DeformConv2d, Transpose, NAM]:
            c1, c2 = ch[f], args[0]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--cfg', type=str, default='DBCR-YOLO.yaml', help='model.yaml')
    ...
    if opt.profile:
        ...
        # img = torch.rand(1,3,1280,1280).to(device)
        y = model(img, profile=True)
    # Tensorboard (not working https://github.com/ultralytics/yolov5/issues/2898)
    # from torch.utils.tensorboard import SummaryWriter
    # tb_writer = SummaryWriter('.')
    # LOGGER.info("Run 'tensorboard --logdir=models' to view tensorboard at http://localhost:6006/")
    # Run the tensorboard "tensorboard --logdir=runs"
