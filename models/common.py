...
class DCConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, g=1, act=True):
        ...
        self.DCconv1 = nn.Conv2d(c1, c1, k, 1, padding=2, dilation=1, groups=g)
        self.DCconv2 = nn.Conv2d(c1, c1, k, 1, padding=2, dilation=2, groups=g)
        self.DCconv3 = nn.Conv2d(c1, c1, k, 1, padding=2, dilation=3, groups=g)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU() if act is True else (act if isinstance(act, nn.Module) else nn.Identity())
        self.conv1 = nn.Conv2d(c1, c2, 3, s, 1)

    def forward(self, x):
        x = self.DCconv3(self.DCconv2(self.DCconv1(x)))
...
class EffectiveSELayer(nn.Module):
    def __init__(self, channel, act=True):
        super(EffectiveSELayer, self).__init__()
        self.fc = nn.Conv2d(channel, channel, kernel_size=1, padding=0)
        # self.act = act if act is None or isinstance(act, (str, dict)) else act
...
class C3New(nn.Module):
    ...
    def __init__(self, c1, c2, n=1, Involution=False, shortcut=True, g=1,
        ...
        self.m = nn.Sequential(*[
            ...
            for _ in range(n)])
        # self.m = nn.Sequential(*[CrossConv(c_, c_, 3, 1, g, 1.0, shortcut) for _ in range(n)])
        self.Attention = NewAttention(c2, c2)
        self.cv4 = Conv(c2, c2, 1, 1)

    def forward(self, x):
        return self.cv4(self.Attention(torch.cat((self.m(self.cv1(x)), self.cv2(x)), dim=1)))


class NewAttention(nn.Module):
    def __init__(self, c1, c2):
        super(NewAttention, self).__init__()
        self.Effect = EffectiveSELayer(c1)
    ...
    def forward(self, x):
        # x = self.cv2(x)
        # print((x+b).shape)
        return x + self.cv1(self.Effect(x) * self.Spatial(x))
...
class Concat_add(nn.Module):
    def __init__(self, c1, c2):
        super(Concat_add, self).__init__()
        self.half = 0.5
        # self.act = nn.ReLU()
        self.cv1 = Conv(c1, c2, 1, 1)
    ...
    def forward(self, x):
        # x[1] = self.CBAM(x[1])
        # print(x[1].shape)
        # print((x[0]).detach().numpy().shape)
        x[0] = self.cv1(x[0])
        x = torch.add(x[0], x[1])

        return self.cv2(self.Coord(x)) + x


class Upsam(nn.Module):
    def __init__(self, c1, c2, k, s):
        super(Upsam, self).__init__()
        self.TransConv = Transpose(c1, c2, k, s)
        self.Coord = CoordAtt(c2, c2)
        self.cv1 = Conv(c2, c2, 3, 1)

    def forward(self, x):
        x[1] = self.TransConv(x[1])
        x1 = torch.add(x[0], x[1])
        x1 = self.Coord(x1)
        return self.cv1(x+x1)
...
class Coord(nn.Module):
    def __init__(self,c1,c2):
        ...
        self.cv3 = Conv(c1, c, 3, 1)
        self.Coordatt = CoordAtt(c1, c1)

    def forward(self, x):
        return torch.cat((self.cv2(x),self.cv3(x + self.Coordatt(x))), dim=1)
...
class ASFFV5_4head(nn.Module):
    def __init__(self, level, multiplier=1, rfb=False, vis=False, act_cfg=True):
        ...
        For even smaller, you need change code manually.
        """
        super(ASFFV5_4head, self).__init__()
        self.level = level
        self.dim = [int(1024 * multiplier), int(768 * multiplier), int(512 * multiplier),
                    int(256 * multiplier)]
        # print(self.dim)

        self.inter_dim = self.dim[self.level]
        if level == 0:
            self.stride_level_1 = Conv(int(768 * multiplier), self.inter_dim, 3, 2)
            ...
            self.stride_level_2 = Conv(int(512 * multiplier), self.inter_dim, 3, 2)
            self.stride_level_3 = Conv(int(256 * multiplier), self.inter_dim, 3, 2)
            self.expand = Conv(self.inter_dim, int(768 * multiplier), 3, 1)
        elif level == 2:
            self.compress_level_0 = Conv(int(1024 * multiplier), self.inter_dim, 1, 1)
            self.compress_level_1 = Conv(int(768 * multiplier), self.inter_dim, 1, 1)
            self.stride_level_3 = Conv(int(256 * multiplier), self.inter_dim, 3, 2)
            self.expand = Conv(self.inter_dim, int(512 * multiplier), 3, 1)
...
class ResBlock(nn.Module):
    def __init__(self, in_place, places, stride=1, downsampling=False, expansion=1):
        super(ResBlock, self).__init__()
        self.expansion = expansion
        self.downsampling = downsampling

        self.bottleneck = nn.Sequential(
            nn.Conv2d(in_channels=in_place, out_channels=places, kernel_size=1, stride=1, bias=False),
            nn.BatchNorm2d(places),
            nn.SiLU(inplace=True),
            nn.Conv2d(in_channels=in_place, out_channels=places, kernel_size=3, stride=stride, padding=1, bias=False),
            nn.BatchNorm2d(places),
        )
        if self.downsampling:
            self.downsampling = nn.Sequential(
                nn.Conv2d(in_channels=in_place, out_channels=places * self.expansion, kernel_size=1, stride=stride,
                          bias=False),
...
class C3Deform(nn.Module):
    # CSP Bottleneck with 3 convolutions
    def __init__(self, c1, c2, n=1, Involution=False, shortcut=True, g=1,
                 e=0.5, expansion=1):  # ch_in, ch_out, number, shortcut, groups, expansion
        super(C3Deform, self).__init__()
        c_ = int(c2 * e)  # hidden channels
        # self.cv1 =  Conv(c1, c1*2, 1, 1) if(Involution==False) else (c1, c1, 1, 1)
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv(c1, c_, 1, 1)
        self.cv3 = DeformConv2d(c2, c2)  # act=FReLU(c2)
        self.expansion = expansion
        self.m = nn.Sequential(*[
            (InvolutionBottleneck(c_, c_, shortcut, g, e=1.0) if Involution else Bottleneck(c_, c_, shortcut, g, e=1.0))
            for _ in range(n)])
...
class Transpose(nn.Module):
    def __init__(self, c1, c2, k, s):
        ...
        self.TrConv = nn.ConvTranspose2d(c1, c2, k, s, padding=1)
        self.bn = nn.BatchNorm2d(c2)
        self.act = nn.SiLU()

    def forward(self, x):
        return self.act(self.bn(self.TrConv(x)))
...
class Spatial_Att(nn.Module):
    def __init__(self, channels):
        ...
        if self.channel == 384:
            self.bn = nn.BatchNorm2d(40, affine=True)
        elif self.channel == 256:
            self.bn = nn.BatchNorm2d(80, affine=True)
        else:
            self.bn = nn.BatchNorm2d(160, affine=True)
...
class NAM(nn.Module):
    def __init__(self, channels, channel):
        ...
        self.channel = Channel_Att(channels)
        # self.spatial = Spatial_Att(channels)

    def forward(self, x):
        x = self.channel(x)
        # self.spatial = Spatial_Att(x.shape[2])
        # x = self.spatial(x)
        return x
